...
def new_chat_members(update: Update, context: CallbackContext) -> None:
    # Pastikan ada anggota baru
    if update.message.new_chat_members:
        new_user = update.message.new_chat_members[0]  # Ambil anggota baru
        id_user = new_user.id  # ID pengguna baru
        id_group = update.message.chat.id  # ID grup
        username = f"@{update.message.chat.username}" if update.message.chat.username else "No data"  # Username grup
        ...
        if new_user.id == context.bot.id:
            ...
            pesan += f"\nadded to <b>{update.message.chat.title}</b> (<code>{id_group}</code>)\n{username}"
